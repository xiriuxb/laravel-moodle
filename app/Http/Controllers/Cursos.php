...
class Cursos extends Controller
{
    ...
    private function getQuery($categoria = null,$visible = 1,$curseid = null, $page=1){
        $courseIdFilter = $curseid == null ? "" : " WHERE shortname = '".$curseid."'";
...
// // SELECT * FROM
// // (SELECT contextid FROM mdl_files WHERE component='course' GROUP BY contextid) mdl_fls 
// // INNER JOIN (SELECT id, instanceid from mdl_context) mdl_cntxt ON mdl_cntxt.id = mdl_fls.contextid 
// // INNER JOIN (SELECT id, category, fullname, shortname, summary FROM mdl_course) mdl_crse ON mdl_crse.id = mdl_cntxt.instanceid
// // INNER JOIN (

//     // SELECT mdl_crse.id, mdl_cntxt.id AS 'context',filename, category,fullname,shortname,summary,precio,value  FROM
//     // (SELECT contextid,filename FROM mdl_files WHERE component='course' GROUP BY contextid) mdl_fls 
//     // INNER JOIN (SELECT id,instanceid from mdl_context) mdl_cntxt ON mdl_cntxt.id = mdl_fls.contextid 
//     // INNER JOIN (SELECT id, category, fullname, shortname, summary FROM mdl_course) mdl_crse ON mdl_crse.id = mdl_cntxt.instanceid
//     // INNER JOIN ( SELECT instanceid, 
//     // MAX(CASe when (fieldid=3) THEN value end) as precio,
//     // MAX(CASE when (fieldid=4) then value end) as value
//     // from mdl_customfield_data GROUP BY instanceid) 
//     // mdl_cstmfld_dta ON mdl_cstmfld_dta.instanceid = mdl_crse.id GROUP BY mdl_crse.id
