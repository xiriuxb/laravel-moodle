<template>
	<div v-if="$page.props.auth.role != 'suspended'">
		...
		<button v-if="!logged" class="btn btn-primary" v-on:click="matricula">
			Inscribirse en el curso ({{ price > 0 ? `$${price}`:`Gratis` }})
		</button>
...
export default {
	components: { PaymentsModal : () => import('./payments/PaymentsModal.vue')},
	layout: [Home, Curso],
	props: {
		curso: {
			type: Object,
	...
	computed: {
		...
		buttonMessage() {
			return this.matriculado
				...
				: "Inscribirse en el curso ($" + this.price + ")";
		},
		moodleUrl() {
	...
	methods: {
		matricula() {
			if (!this.logged) {
				this.$toast.open({ message: "Debe registrarse e ingresar para poder matricularse", position: "top", type: "warning", });
			} else {
				if (!this.verificado) {
		...
		redirectToMoodle() {
			window.open(this.moodleUrl+"course/view.php?name=" + this.curso.shortname);
		},
		redirectOrSignup() {
